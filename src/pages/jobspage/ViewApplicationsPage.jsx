    ...
    fetch("/job/" + jobId)
      ...
      .then(({ data: job }) =>
        setJob(() => ({
          ...job,
          appliedPeople:
            job && job.appliedPeople
              ? job.appliedPeople.split(" ").filter((x) => x.length)
              : [],
          reqSkill: job.reqSkill ? arrayFromString(job.reqSkill) : [],
        }))
      );
    ...
    <div className="jobs-page">
      ...
      <section className="page-section flex flex-col gap-4">
        <h1 className="text-3xl font-bold">
          Applications recieved for the Job
        </h1>
        {job && <JobCard job={job} />}
        ...
        {!loading && (
          <div className="flex mt-5 gap-4 flex-wrap flex-col items-start bg-purple-rgba px-12 py-4">
            {applications.length > 0 ? (
              <>
                <h3 className="flex-1">
                  People who have applied for this job post :
                </h3>
